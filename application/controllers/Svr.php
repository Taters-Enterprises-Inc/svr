...
class Svr extends CI_Controller {
    ...
    public function index()
    {
        ...
        if ($this->session->has_userdata('is_login')) {
            $query = $this->db->get('svr_files');
            $data['svr_files'] = $query->result();
    ...
    public function generate_svr_for_22020882()
    {
        ini_set("soap.wsdl_cache_enabled", "0");
        $client = new SoapClient("http://tei.alliancewebpos.com/appserv/app/w3p/w3p.wsdl", array("location" => "http://tei.alliancewebpos.com/appserv/app/w3p/W3PSoapServer.php"));
        ...
        // Subtract 7 days from the current date for the start date of the previous week
        $startDate = $currentDate->modify('-7 days');

        // Create a new DateTime object for the end date and subtract 6 days from the start date
        ...
        // Set headers for the output Excel file
        $filenameDate = date("F j, Y", strtotime($fromDate)) . " - " . date("F j, Y", strtotime($toDate));
        $filename = "Sales Variance Report as of " . $filenameDate . ".xlsx";

        // Define the folder path
        $folderPath = './assets/svr/22020882/';
        ...
        $svr_files_data = array(
            'folder_path' => $folderPath,
            'name_of_file' => $filename,
            'store_name' => 'TEI - SM MOA Skating',
            'from_date' => $fromDate,
            'to_date' => $toDate,
        );
        $this->db->insert('svr_files', $svr_files_data);
